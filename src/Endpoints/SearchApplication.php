...
class SearchApplication extends AbstractEndpoint
{
	...
	{
		$url = $this->encode("/_application/search_application/{$name}");
		$method = 'DELETE';
	...
	{
		$url = $this->encode("/_application/analytics/{$name}");
		$method = 'DELETE';
		$url = $this->addQueryString($url, $params, ['pretty', 'human', 'error_trace', 'source', 'filter_path']);
		$headers = [
		    'Accept' => 'application/json',
	...
	{
		$url = $this->encode("/_application/search_application/{$name}/_search");
